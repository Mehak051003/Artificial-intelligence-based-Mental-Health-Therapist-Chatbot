    ...
    def run_chat_bot(self, user_input, user_provided_answer=None):
        ...
        if best_match:
            ...
            if user_provided_answer:
                # Save the user-provided answer to the knowledge base
                self.knowledge_base["questions"].append({"Question": user_input, "Answer": user_provided_answer})
                self.question_dict = self.build_question_dict(self.knowledge_base["questions"])
                self.save_knowledge_base('knowledge_base.json', self.knowledge_base)
                return 'Bot: Thank you! I learned something today'
            else:
                return 'Bot: I don\'t know the answer. You can rephrase your prompt or you can teach me? '
...
def main():
    ...
    if user_input:
        ...
        # If the bot prompts for a user-provided answer, display an input field
        if "Bot: Thank you! I learned something today" in bot_response:
            user_provided_answer = st.text_input("Type the answer:")
            if user_provided_answer:
                bot_response = chatbot.run_chat_bot(user_input, user_provided_answer)
                st.write(bot_response)
